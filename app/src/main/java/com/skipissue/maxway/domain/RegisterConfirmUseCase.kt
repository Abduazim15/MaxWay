...
class RegisterConfirmUseCase @Inject constructor(private val repository: ProductsRepository,private val settings: Settings) {
    suspend operator fun invoke(entity: RegisterConfirmEntity): State {
        try {

            return State.Success(repository.registerConfirm(settings.shipperId!!, entity).body())
        } catch (e: Exception) {
            e.printStackTrace()
