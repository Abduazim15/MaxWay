...
class GetFillialsUseCase @Inject constructor(private val repository: ProductsRepository,private val settings: Settings) {
    suspend operator fun invoke(): State {
        try {

            return State.Success(repository.getFillials(settings.shipperId!!).body())
        } catch (e: Exception) {
            e.printStackTrace()
