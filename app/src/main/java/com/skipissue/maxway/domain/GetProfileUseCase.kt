...
class GetProfileUseCase @Inject constructor(private val repository: ProductsRepository,private val settings: Settings) {
    suspend operator fun invoke(): State {
        try {

            return State.Success(repository.getProfile(settings.accessToken!!).body())
        } catch (e: Exception) {
            e.printStackTrace()
