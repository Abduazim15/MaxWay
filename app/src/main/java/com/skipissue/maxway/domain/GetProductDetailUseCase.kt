...
class GetProductDetailUseCase @Inject constructor(private val repository: ProductsRepository,private val settings: Settings) {
    suspend operator fun invoke(productId: String): State {
        try {

            return State.Success(repository.getProductDetail(settings.accessToken!!, productId).body())
        } catch (e: Exception) {
            e.printStackTrace()
